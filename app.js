...
const resultEmailSimulation = window.RESULT_EMAIL_SIMULATION !== false;
...
function recordSimulatedEmail(payload) {
  const record = {
    simulatedAt: new Date().toISOString(),
    deliveryRoute: payload.notification.route || payload.delivery.route,
    replyTo: payload.notification.replyTo,
    subject: payload.notification.subject,
    contact: payload.contact,
    assessment: payload.assessment,
    axisResults: payload.axisResults,
    answers: payload.answers,
    notificationText: payload.notification.text,
  };

  window.__pti2030LastSimulatedEmail = record;

  try {
    const key = "pti2030_email_simulation_outbox";
    const current = JSON.parse(window.localStorage.getItem(key) || "[]");
    current.push(record);
    window.localStorage.setItem(key, JSON.stringify(current));
    return { count: current.length, stored: true };
  } catch (error) {
    return { count: 1, stored: false };
  }
}
...
async function registerAssessmentResult() {
  ...
  if (!resultRegistrationEndpoint) {
    if (resultEmailSimulation) {
      const simulated = recordSimulatedEmail(payload);
      state.finalized = true;
      updateGenerateState();
      const storageText = simulated.stored ? ` Registro local #${simulated.count}.` : " Registro disponible en la sesión actual.";
      setBatchStatus(`Autodiagnóstico enviado correctamente.${storageText}`, "success");
    ...
    updateGenerateState();
    setBatchStatus("No se pudo completar el envío automático. Se ha descargado un respaldo local del informe.", "error");
    showToast("Envío automático no configurado.");
    return;
  }
